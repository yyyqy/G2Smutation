...
public class PdbScriptsPipelineApiToSQL {
    ...
    /** 
     * Call millions of API from genomenexus, Now it supports 5 Annotations
     * @return
     */
    public int generateAllSNPMappingSQLfile(String mappingAnnotationName) {
        List<String> tempLines = new ArrayList<String>();
        int sql_insert_output_interval = Integer.parseInt(ReadConfig.sqlInsertOutputInterval);
        int fileCount = 0;
        String sqlfilepwd = new String(ReadConfig.workspace + ReadConfig.rsSqlInsertFile + "." + fileCount);
        File rssqlfile = new File(sqlfilepwd);
        List<String> snpIds = new ArrayList<String>();
        
        switch (mappingAnnotationName) {
        case "dbsnp":
            snpIds = getDbSNPIdFromMappingFile(ReadConfig.workspace + ReadConfig.dbsnpFile);
            break;
        case "clinvar":
            break;
        case "cosmic":
