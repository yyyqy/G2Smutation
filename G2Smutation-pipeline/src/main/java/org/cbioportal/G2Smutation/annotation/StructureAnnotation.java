...
public class StructureAnnotation {
    ...
    public String getNaccessInfo(String pdbId, String pdbChain, String pdbResidueIndex){
		...
		try {
			...
			int i = 0;
			for(;i<lines.size();i++) {
				if(!(lines.get(i).substring(0,3).equals("RES")&&lines.get(i).substring(8,9).equals(pdbChain)&&(stringToInt(lines.get(i).substring(9,13))==Integer.parseInt(pdbResidueIndex)))) {
					continue;
				}
				else {
					break;
