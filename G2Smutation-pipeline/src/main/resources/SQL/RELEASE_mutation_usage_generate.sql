...
CREATE TABLE `mutation_usage_table` (
  ...
  KEY(`MUTATION_NO`,`SEQ_ID`,`PDB_NO`,`ALIGNMENT_ID`,`PDB_RESIDUE`),
...
SELECT DISTINCT x.*,z.IDENTITY,z.IDENTITYP,z.EVALUE,z.BITSCORE,z.ALIGNLENGTH  FROM mutation_entry x JOIN mutation_entry y JOIN pdb_seq_alignment z ON y.MUTATION_NO = x.MUTATION_NO AND y.PDB_RESIDUE <> x.PDB_RESIDUE AND z.ALIGNMENT_ID = x.ALIGNMENT_ID ORDER BY MUTATION_NO;
