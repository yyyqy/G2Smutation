...
public class MainController {
    ...
    /*
     * @GetMapping("/statistics") public ModelAndView statisticsInfo(Model
     * model) { List<Statistics> statistics =
     * statisticsRepository.findTop2ByOrderByIdDesc(); return new
     * ModelAndView("/statistics", "statistics", statistics); }
     */
    ...
    public ModelAndView rssearchGetSEQUNION(HttpServletRequest request, Model model) {
        String SEQID = request.getParameter("SEQID");
        String SEQINDEX = request.getParameter("SEQINDEX");
        List<rs_mutation_entry> rsdatafind = RsRepository.findByseqIdAndSeqIndex(Integer.parseInt(SEQID),
                Integer.parseInt(SEQINDEX));
    ...
    public ModelAndView rssearchPageGet(HttpServletRequest request, Model model) {
        ...
        if (number >= rssearchMaxPage)
            number = rssearchMaxPage;
    ...
    public ModelAndView searchGetDEQID(HttpServletRequest request, Model model) {
        String SEQID = request.getParameter("SEQID");
        List<mutation_usage_table> datafind = mutationRepository.findByseqId(Integer.parseInt(SEQID));
    ...
    public ModelAndView searchGetSEQUNION(HttpServletRequest request, Model model) {
        String SEQID = request.getParameter("SEQID");
        String SEQINDEX = request.getParameter("SEQINDEX");
        List<mutation_usage_table> datafind = mutationRepository.findByseqIdAndSeqIndex(Integer.parseInt(SEQID),
                Integer.parseInt(SEQINDEX));
    ...
    public ModelAndView searchGetpdbUNION(HttpServletRequest request, Model model) {
        String PDBNO = request.getParameter("PDBNO");
        String PDBINDEX = request.getParameter("PDBINDEX");
        List<mutation_usage_table> datafind = mutationRepository.findBypdbNoAndPdbIndex(PDBNO,
                Integer.parseInt(PDBINDEX));
    ...
    /*
     * @RequestMapping("/search/unionSearch") public ModelAndView
     * searchGetSEQRESIDUE(HttpServletRequest request,Model model){ String
     * SEQRESIDUE = request.getParameter("SEQRESIDUE"); String PDBINDEX =
     * request.getParameter("PDBINDEX"); String PDBRESIDUE =
     * request.getParameter("PDBRESIDUE"); backSEQRESIDUE=SEQRESIDUE;
     * backPDBINDEX=PDBINDEX; backPDBRESIDUE=PDBRESIDUE;
     * model.addAttribute("backSEQRESIDUE", SEQRESIDUE);
     * model.addAttribute("backPDBINDEX", PDBINDEX);
     * model.addAttribute("backPDBRESIDUE", PDBRESIDUE);
     * 
     * List<mutation_usage_table> datafind;
     * 
     * if (PDBINDEX.equals("") && PDBRESIDUE.equals("")) {
     * datafind=mutationRepository.findBySeqResidueOrderByPdbIndex(SEQRESIDUE);}
     * else if (PDBINDEX.equals("") && SEQRESIDUE.equals("")) {
     * datafind=mutationRepository.findBypdbResidueOrderByPdbIndex(PDBRESIDUE);}
     * else if (SEQRESIDUE.equals("") && PDBRESIDUE.equals("")) {
     * datafind=mutationRepository.findBypdbIndex(Integer.parseInt(PDBINDEX));}
     * 
     * else if (SEQRESIDUE.equals("")) {
     * datafind=mutationRepository.findBypdbIndexAndPdbResidue(Integer.parseInt(
     * PDBINDEX),PDBRESIDUE);} else if (PDBINDEX.equals("")) {
     * datafind=mutationRepository.findByseqResidueAndPdbResidue(SEQRESIDUE,
     * PDBRESIDUE);} else if (PDBRESIDUE.equals("")) {
     * datafind=mutationRepository.findBypdbIndexAndSeqResidue(Integer.parseInt(
     * PDBINDEX),SEQRESIDUE);}
     * 
     * else
     * {datafind=mutationRepository.findBySeqResidueAndPdbResidueAndPdbIndex(
     * SEQRESIDUE,PDBRESIDUE,Integer.parseInt(PDBINDEX));}
     * 
     * searchData=datafind; totalCount = searchData.size(); Integer requestCount
     * = totalCount / searchPerPage+1; searchMaxPage = Math.max(1,
     * requestCount); model.addAttribute("requestCount", searchMaxPage);
     * model.addAttribute("totalCount", totalCount);
     * 
     * // First page Integer number = 1; int toIndex =
     * Math.min(totalCount,searchPerPage); List<mutation_usage_table> subList =
     * datafind.subList(0, toIndex);
     * 
     * model.addAttribute("number",number); model.addAttribute("data", subList);
     * return new ModelAndView("databasesearch"); }
     */
    ...
    public ModelAndView searchPageGet(HttpServletRequest request, Model model) {
        ...
        if (number >= searchMaxPage)
            number = searchMaxPage;
